...
const page = usePage();
const form = useForm({
    name: page.props.team.name,
});

const submit = () => {
    form.post(route('teams.update', page.props.team.id), {
        onError: (errors) => {
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form @submit.prevent="submit" enctype="multipart/form-data" class="mt-2 p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input class="w-full border-0 rounded-md ring-gray-300" v-model="form.name" name="name" id="name" type="text">
                            <span class="text-red-500 text-xs">{{ form.errors.name }}</span>
                        </div>
                        <Link href="/teams" class="text-blue-500 hover:text-blue-700">Back</Link>
